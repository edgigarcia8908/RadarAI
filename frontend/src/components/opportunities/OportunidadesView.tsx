  ...
  return (
    <button className={`opportunity-card opportunity-card-${item.tone}`} type="button" onClick={onToggle} aria-expanded={abierto}>
      ...
      <span className="opportunity-data">
        ...
        {abierto && (
          <span style={{ display: 'block', marginTop: 8, fontSize: 13, textAlign: 'left' }}>
            <strong>Por qué es compatible:</strong>
            <ul style={{ margin: '4px 0 0', paddingLeft: 18 }}>
              {item.porQue.map((razon, i) => <li key={i}>{razon}</li>)}
            </ul>
      ...
      <HomeIcon name={abierto ? 'chevron-right' : 'chevron-right'} size={17} />
    </button>
  );
}
  ...
  return (
    <div className="opportunities-page">
      ...
      <section className="opportunities-form" aria-labelledby="opportunities-title">
        ...
        {isSearching && (
          <ul style={{ listStyle: 'none', padding: 0, fontSize: 13, color: '#444', marginTop: 8 }}>
            <li>{paso === 'preparando' ? '⏳' : '✅'} Preparando perfil y sincronizando SECOP</li>
            <li style={{ opacity: paso === 'calculando' ? 1 : 0.5 }}>{paso === 'calculando' ? '⏳' : paso === null ? '✅' : '○'} Calculando compatibilidad</li>
          </ul>
        )}
      </section>
